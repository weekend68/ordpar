...
```
Denna data skickas med i nästa AI-prompt för den användaren: "Undvik liknande kopplingar"
...
- Komprimera till kategorier: "Användaren gillar INTE matematiska kopplingar"
- Skicka bara de 20 senaste dåliga grupperna till AI
- Resten aggregeras till högre nivå
...
- **Databas:** PostgreSQL eller SQLite (MVP)
- **AI:** Claude API (Sonnet 4)
- **Hosting:** Enklast möjliga (t.ex. Vercel + Supabase)
...
- Ingen real-time sync (polling var 2-3 sek är OK)
- Ingen fancy animation (bara shake på fel gissning)
...
|------|-------------|---------|------------|
| AI genererar dåliga ord | Hög | Hög | Manuell QA första veckorna, feedback-loop, träna på DN-facit |
...
| För långsamt/buggigt | Medel | Hög | Håll MVP enkelt, testa på riktiga enheter |
| Ingen vill spela | Låg | Hög | Testa med vänner först innan utbyggnad |
| Feedback-data blir för stor | Medel | Låg | Komprimera till kategorier, skicka bara senaste 20 |
...
3. Generera 10 nya testset med Claude
4. Jämför kvalitet mot DN
5. Justera prompt baserat på resultat

**Spår 2: Teknisk implementation**
1. Bygg minimal frontend → Visa 16 ord, turbaserat klickande
2. Bygg backend → Spara spel, hantera state
3. Integrera Claude API → Generera sets on-demand
4. Lägg till feedback-loop → Samla data på vad som funkar
5. Implementera poängsystem
...
**Status:** Draft v0.2  
**Uppdaterat med:** Fel-hantering, poängsystem, feedback-skillnader, återanvändning av ordgrupper, parallella utvecklingsspår  
